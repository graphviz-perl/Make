...
my ($other_rule) = @{ $m->target('other')->rules($m->phony('other')) };
...
my ($all_rule) = @{ $all_target->rules($m->phony('all')) };
...
for my $tuple ( [ undef, undef ], [ undef, 'subdir' ], [qw(GNUmakefile subdir)] ) {
    ...
    $m->parse($mf);
    $got = $m->target('all')->rules($m->phony('all'))->[0]->prereqs;
    is_deeply $got, [qw(a.o b.o)] or diag explain $got;
    $got = $m->target('a.o')->rules($m->phony('a.o'))->[0]->prereqs;
