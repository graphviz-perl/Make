...
my ($other_rule) = @{ $m->target('other')->rules($m->phony('other'), 'other', $m) };
...
my ($all_rule) = @{ $all_target->rules($m->phony('all'), 'all', $m) };
...
my ($ax_rule) = @{ $ax_target->rules($m->phony('a.x.o'), 'a.x.o', $m) };
...
for my $tuple ( [ undef, undef ], [ undef, 'subdir' ], [qw(GNUmakefile subdir)] ) {
    ...
    $m->parse($mf);
    $got = $m->target('all')->rules($m->phony('all'), 'all', $m)->[0]->prereqs;
    is_deeply $got, [qw(a.o b.o)] or diag explain $got;
    $got = $m->target('a.o')->rules($m->phony('a.o'), 'a.o', $m)->[0]->prereqs;
